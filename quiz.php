...
// Cek apakah user sudah memilih kelompok atau belum
...
<head>
    ...
    <style>
        ...
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .container { background: #fff; border-radius: 24px; box-shadow: 0 20px 50px rgba(236, 72, 153, 0.15); width: 100%; max-width: 500px; padding: 40px 30px; text-align: center; position: relative; overflow: hidden; margin-bottom: 20px; }
        ...
        /* CSS untuk Menu Pilih Kelompok */
        .group-grid { display: grid; grid-template-columns: 1fr; gap: 15px; margin-top: 20px; }
        .group-card { display: block; background: #fce7f3; border: 2px solid #f9a8d4; padding: 20px; border-radius: 16px; text-decoration: none; color: var(--primary-dark); transition: all 0.3s ease; }
        .group-card:hover { background: var(--primary); color: #fff; border-color: var(--primary); transform: translateY(-3px); box-shadow: 0 10px 20px rgba(236, 72, 153, 0.2); }
        .group-card h3 { font-size: 1.2rem; margin-bottom: 5px; }
        .group-card p { font-size: 0.8rem; opacity: 0.9; }
        ...
        .copyright { margin-top: 30px; font-size: 0.75rem; color: #db2777; opacity: 0.7; font-weight: 600; letter-spacing: 0.5px; }
    </style>
</head>
<body>
    ...
    <div class="container">
        ...
        <?php if ($selected_group === 0): ?>
            <!-- HALAMAN MENU PILIH KELOMPOK -->
            <h1>🌸 Pilih Kelompok</h1>
            <p class="subtitle">Pilih kelompok kosakata yang ingin kamu pelajari</p>
            
            <div class="group-grid">
                <a href="quiz.php?group=1" class="group-card">
                    <h3>Kelompok 1 (どうし 1)</h3>
                    <p>70 Kosakata Kata Kerja Dasar</p>
                </a>
                <a href="quiz.php?group=2" class="group-card">
                    <h3>Kelompok 2 (どうし 2)</h3>
                    <p>37 Kosakata Kata Kerja Lanjutan</p>
                </a>
                <a href="quiz.php?group=3" class="group-card">
                    <h3>Kelompok 3 (どうし 3)</h3>
                    <p>27 Kosakata Kata Kerja Suru</p>
                </a>
            </div>
            <div class="copyright">© Copyright by sultbabies</div>

        <?php else: 
            $groupName = "Kelompok " . $selected_group;
        ?>
            <!-- HALAMAN GAME KUIS -->
            ...
            <div class="score-board">
                ...
                <div>Soal: <span id="question-num">1</span> / <span id="total-q"></span></div>
            </div>
    ...
    <!-- Modal Alert (Hanya muncul saat game berjalan) -->
    <?php if ($selected_group !== 0): ?>
    ...
    <div class="modal-overlay" id="finishModal">
        <div class="modal-box" style="border-top-color: var(--success);">
            ...
            <a href="quiz.php?group=<?= $selected_group ?>" class="modal-btn">Main Lagi</a>
            <a href="quiz.php" class="modal-btn success">Ganti Kelompok</a>
    ...
    <script>
        ...
        const vocabData = selectedGroup === 1 ? group1 : (selectedGroup === 2 ? group2 : group3);
        ...
        totalQEl.textContent = vocabData.length;
        ...
        function handleTimeout() {
            if(answered) return;
            answered = true;
            wrongAnswers.push({ question: currentQuestion.id, correct_answer: currentQuestion.jp, user_answer: "Waktu Habis" });
        ...
        function loadQuestion() {
            if (currentQuestionIndex >= quizPool.length) { finishGame(); return; }
            answered = false;
            currentQuestion = quizPool[currentQuestionIndex];
        ...
        function checkAnswer(selectedOption, btn) {
            ...
            if(selectedOption.jp === currentQuestion.jp) {
                ...
                correctAnswerEl.textContent = currentQuestion.jp;
                wrongAnswers.push({ question: currentQuestion.id, correct_answer: currentQuestion.jp, user_answer: selectedOption.jp });
        ...
        function finishGame() {
            stopTimer();
            finalScoreEl.textContent = score + " / " + quizPool.length;
            ...
            fetch('save_score.php', {
                ...
                body: JSON.stringify({ 
                    ...
                    group_type: selectedGroup, // Kirim info kelompok ke server
        ...
        quizPool = shuffleArray([...vocabData]);
        loadQuestion();
    </script>
    <?php endif; ?>
